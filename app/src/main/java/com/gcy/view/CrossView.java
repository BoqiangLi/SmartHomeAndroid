...
public class CrossView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(mState==FLAG_STATE_PLUS){
            mPaintCir.setColor(Color.rgb(56,164,178));
            mPaintCir.setStyle(Paint.Style.STROKE);
            mPaintCir.setStrokeWidth(3);
            canvas.drawCircle((mFromXY[0]+mToXY[0])/2,(mFromXY[1]+mToXY[1])/2,(mFromXY[1]+mToXY[1])/2,mPaintCir);
        }else{
            mPaintCir.setColor(Color.rgb(56,164,178));
    ...
    public int toggle(long animationDurationMS) {
        ...
        if(mPaint==null)
            mPaint = new Paint();
        if(mState == FLAG_STATE_PLUS){
            mPaint.setColor(Color.rgb(56,164,178));
        }else{
